...
- `tests/` — executable checks over corpus, real egui output, identity, diffs, backpressure, custom paint, and live protocol framing.
...
- kind, bounds, clip, verification, layer, or other material binding changes remain material;
...
`diff-exact --yaml` emits the structured correlated diff. Agent text is the default and explicitly labels material authored changes, object/binding ambiguity, and non-material execution-handle churn.
...
Blocking networking remains on worker threads. The Canvas page deliberately mixes explicitly authored objects with anonymous background/text paint so instrumentation does not pretend to semanticize the whole renderer.
...
- a living eframe showcase with worker-hosted `:5720` and `:5721` services;
...
The next pressure is narrower now: **real agent debugging loops against the showcase**, safe mapping (if any) from layer-local authored bindings to flattened renderer order, richer keyed live-showcase examples, and stronger raster/paint evidence before any canonical occlusion claim.
